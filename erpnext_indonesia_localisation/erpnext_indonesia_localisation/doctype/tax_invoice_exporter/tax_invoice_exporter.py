...
import erpnext_indonesia_localisation.utils.constants as constants

from erpnext_indonesia_localisation.utils.data import cint, get_tax_prefix_code
...
class TaxInvoiceExporter(Document):
	...
	def update_tin_with_si(self, event_trigger):
		...
		if "submit" in event_trigger:
			...
			queue_name = "long"
			if indonesia_localisation_settings.worker_for_link_tax_invoice_number_background_job:
				queue_name = indonesia_localisation_settings.worker_for_link_tax_invoice_number_background_job
				...
				if self.assign_tax_invoice_number_to_multiple_invoices:
					...
					if tie_si.tax_invoice_number not in list_of_tin:
						...
						enqueue(
							...
							queue=indonesia_localisation_settings.worker_for_renaming_tax_invoice_number if indonesia_localisation_settings.worker_for_renaming_tax_invoice_number else "short",
							is_async=True,
	...
	def get_si_without_tin(self):
		...
		indonesia_localisation_settings_doc = frappe.get_single("Indonesia Localisation Settings")

		if indonesia_localisation_settings_doc.exclude_opening_entry:
		...
		if indonesia_localisation_settings_doc.exclude_sales_invoice_type_return:
		...
		for invoice_row in self.sales_invoices:
			if not invoice_row.is_invoice_cancelled or not invoice_row.is_tin_unlinked_from_invoice:
				...
				if invoice_row.customer not in customer_tax:
					...
					if indonesia_localisation_settings.invoice_name_in_efaktur_template:
						invoice_name = si_doc.as_dict()[indonesia_localisation_settings.invoice_name_in_efaktur_template]
					else:
					...
					if indonesia_localisation_settings.no_faktur_format == 0:
